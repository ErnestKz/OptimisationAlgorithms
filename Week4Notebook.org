...
from OptimisationAlgorithmToolkit.Plotting import ploty
#+end_src
...
ada_out = Adagrad.set_parameters(
    x0=[1],
    f=f_quadratic,
    iters=4,
    alpha0=[0.3],               # this should determine each records label
    eps=1e-5).run()
rms_out = RMSProp.set_parameters(
    x0=[1],
    f=f_quadratic,
    iters=4,
    alpha0=0.3,
    beta=0.1,
    eps=1e-5).run()
comb_out = rms_out + ada_out

# ada_out = Adagrad.set_parameters(
#     x0=[1],
#     f=f_quadratic,
#     iters=50,
#     alpha0=0.2,               # this should determine each records label
#     eps=1e-5).run()
# rms_out = RMSProp.set_parameters(
#     x0=[1],
#     f=f_quadratic,
...
create_labels(comb_out)
t = get_titles(comb_out )
# t = get_titles(comb_out, ['x0', 'iters'])

ax = ploty(comb_out)
ax.set_title(f'{t["Adagrad"]} \n {t["RMSProp"]}')
#+end_src

#+RESULTS:
:RESULTS:
: Text(0.5, 1.0, 'Adagrad: x0=[1] iters=4 eps=1e-05 alpha0=0.3 \n RMSProp: beta=0.1 iters=4 alpha0=0.3 x0=[1] eps=1e-05')
[[file:./.ob-jupyter/255aed1ec036568a7f06bf8e0f8d9fa3b5fc6175.png]]
:END:
