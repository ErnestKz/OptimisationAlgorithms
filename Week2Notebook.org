...
- Figure [[fig:fx4]] plots the function to be optimised; it is convex, but there is a very flat portion at $-0.5 < x < 0.5$. We know that the $argmin_x f(x)=0$ for this convex function.
...
- Figure [[fig:yi]] plots $y$ against $i$; the majority of the optimisation happens in 2 iterations, and very little progress is made after $i=2$, it essentially comes to a flat line 10 iterations onward.
We see that $x_i$ takes longer to become a flat line than $y(x_i)$, this is because of the flat shape of the bottom of $x^4$, once $x_i$ reaches the bottom, $x_i$ itself can still move a bit it will not have a equally proportional impact on $y(x_i)$.
...
- Varying $x_0$
  - Plotting x: [[fig:varx0x]]
  - Plotting y: [[fig:varx0y]]
  - Plotting y: [[fig:varx0ynon]]

- Varying $\alpha$
  - Plotting x: [[fig:varax]]
  - Plotting y: [[fig:varay]]
